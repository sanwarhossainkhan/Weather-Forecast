...
{{--            <a href="{{ route('weather_forecast_data_city',[$apidata->city]) }}" style="text-decoration: none;color: #1a202c">--}}
            <div class="card mb-3 shadow-sm p-2 mb-5 bg-body rounded" style="max-width: 26rem;">
                <div class="card-header fw-bolder fs-4 d-flex justify-content-between">
                    ...
                    <div><i class="fa-solid fa-temperature-three-quarters"></i> {{ $datadecode->main->temp }}</div>
                </div>
                <div class="card-body">
                    <h5 class="card-title text-center">{{ $datadecode->weather[0]->main }}</h5>
                    <div class="card-text d-flex justify-content-between">
                        <div data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Current High Temp."><i class="fa-solid fa-temperature-high text-danger"></i> {{ $datadecode->main->temp_max }} </div>
                        <div data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Current Low Temp."><i class="fa-solid fa-temperature-low text-warning"></i> {{ $datadecode->main->temp_min }} </div>
                        <div data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Current Feel Temp."><i class="fa-solid fa-temperature-half text-success"></i> {{ $datadecode->main->feels_like }} </div>
                        <div data-bs-toggle="tooltip" data-bs-placement="bottom" data-bs-title="Current Humidity"><i class="fa-solid fa-snowflake text-primary"></i> {{ $datadecode->main->humidity }} </div>
                    </div>
